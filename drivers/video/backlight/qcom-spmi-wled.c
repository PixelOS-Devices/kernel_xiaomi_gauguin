...
static int wled5_set_brightness(struct wled *wled, u16 brightness)
{
	...
	static u32 prop_read, cabc_disable, cabc_off_dbv, cabc_pwm[2];
	...
	if (!prop_read) {
		...
		if (rc) {
			pr_debug("wled config failed to read mi,cabcoff-dbv or mi,cabc-pwm\n");
			cabc_off_dbv = 0;
		} else
			pr_debug("wled config dbv:%d, pwm:%d:%d", cabc_off_dbv, cabc_pwm[0], cabc_pwm[1]);
